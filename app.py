...
# [⚠️ 핵심 수정] 모든 모드에 "순수 한국어 표준어만 사용" 지침을 강력하게 주입했습니다.
language_guard = (
    " 반드시 완벽한 표준 한국어로만 답변해라. "
    "절대로 '变量', 'の' 같은 중국어 한자, 일본어, 혹은 어색한 번역투 문장을 섞지 마라. "
    "한국어 학술 표준 용어(예: 매개변수, 조절효과, 인과관계 등)만 정확하게 사용해야 한다."
)
...
if "순한맛" in taste:
    system_instruction = (
        ...
        f"비속어는 절대 쓰지 말고 지적인 도발이어야 해.{language_guard} "
    ...
    system_instruction = (
        ...
        f"절대로 욕설이나 비속어는 쓰지 마. 오직 고급 유머와 드립으로만 승부해줘!{language_guard} "
        "CRITICAL: 제목 외에 '이 제목은~' 같은 설명, 서론, 인사말은 절대로 하지 마. 오직 제목 3개만 딱 출력해."
    )
else:
    system_instruction = (
